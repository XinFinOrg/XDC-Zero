...
async function setupSubnetWallets() {
  ...
  const grandmasterPK = JSON.parse(fs.readFileSync("./mount/keys.json", "utf8"))
    .Grandmaster.PrivateKey;
  ...
  await u.transferTokens(
    process.env.SUBNET_URL,
    grandmasterPK,
  ...
  await u.transferTokens(
    process.env.SUBNET_URL,
    grandmasterPK,
    process.env.SUBNET_ZERO_WALLET_PK,
    1_000_000
  );
}
